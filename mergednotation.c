...
int main(int argc, char *argv[])
{
    ...
    if ((strcmp(input_format, "infix") == 0) && (strcmp(output_format, "postfix") == 0))
    { // Infix to postfix
        char postfix[100] = {0};
        infix_to_postfix(expression, postfix);
    }
    else if ((strcmp(input_format, "infix") == 0) && (strcmp(output_format, "prefix") == 0))
    { // Infix to prefix
        char prefix[100] = {0};
        infix_to_prefix(expression, prefix);
    }
    else if ((strcmp(input_format, "prefix") == 0) && (strcmp(output_format, "infix") == 0))
    { // Prefix to infix
        int index = 0;
...
int isInfix(const char *infix)
{
    ...
    for (int i = 0; infix[i] != '\0'; i++)
    {
        char token = infix[i];
        if (isASpace(token))
            continue; // If space, continue to next character
        if (token == '(')
        {
            balance++;
        }
        else if (token == ')')
        {
            balance--;
            if (balance < 0)
                return 0; // Too many closing parentheses (without a matching opening parentheses)
        }
        else if (isOperand(token))
        {
            if (!expectOperand)
                needOperator = 1; // Two operands in a row (expectOperand = 0)
            expectOperand = 0;    // Next, expect an operator
        }
        else if (isOperator(token))
        {
            if (expectOperand)
                return 2;      // Operator without operand (expectOperand = 1)
...
void infix_to_postfix(const char *infix, char *postfix)
{
    ...
    for (int i = 0; infix[i]; i++)
    {
        ...
        {
            if (tokenCount > 0)
                postfix[j++] = ' '; // Add space before token if not first
            postfix[j++] = token;
            tokenCount++;
        ...
        else if (token == ')')
        {
            ...
            {
                if (tokenCount > 0)
                    postfix[j++] = ' '; // Add space before operator
                Node *node = pop(&opStack);
...
int isPrefix(const char *prefix)
{
    ...
    for (int i = len - 1; i >= 0; i--)
    {
        char token = prefix[i];
        if (isASpace(token))
            continue; // If space, proceed to next character
    ...
    if (operatorCount > operandCount || (operatorCount == 1 && operandCount == 1)) // If equal or operator is greater
        return 1;                                                                  // The expression is valid
    else if (operatorCount + 1 > operandCount)
        return 2; // If operator + 1 is greater, insufficient operand
...
Node *prefix_to_tree(char *prefix, int *index)
{
    ...
    if (validPrefix == 1)
    {
        ...
        if (token == '\0')
            return NULL; // If token is null terminator, return NULL
        ...
        return node; // Return node if operand
    }
...
int isPostfix(const char *postfix)
{
    ...
    for (int i = 0; postfix[i] != '\0'; i++)
    {
        ...
        if (isASpace(token))
            continue; // Proceed to next character if space
...
Node *postfix_to_tree(char *postfix)
{
    ...
    if (validPostfix == 1)
    {
        ...
        for (int i = 0; postfix[i] != '\0'; i++)
        {
            ...
            if (isASpace(token))
            { // If token is a space, proceed to next character
                continue;
            }
            ...
            if (isOperator(token))
            { // If token is an operator, pop two nodes
